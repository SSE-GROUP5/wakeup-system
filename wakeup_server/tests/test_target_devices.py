...
def test_empty_name(client):
    response = client.post("/target_devices", json={"matter_id": "switch.fake-matter-device", "name": "", "type": "switch"})
...
def test_empty_type(client):
    response = client.post("/target_devices", json={"matter_id": "switch.fake-matter-device", "name": "Fake Matter Device", "type": ""})
...
def test_too_long_name(client):
    obj = client.post("/target_devices", json={"matter_id": "switch.fake-matter-device", "name": "a"*1000, "type": "switch"}).get_json()
    with pytest.raises(TypeError) as e:
        name = obj['name']
    msg = e.value.args[0]
...
def test_too_long_type(client):
    obj = client.post("/target_devices", json={"matter_id": "switch.fake-matter-device", "name": "Fake Matter Device", "type": "a"*1000}).get_json()
    with pytest.raises(TypeError) as e:
        type = obj['type']
    msg = e.value.args[0]
...
def test_duplicate_name(client):
    response1 = client.post("/target_devices", json={"matter_id": "switch.fake-matter-device", "name": "Fake Matter Device", "type": "switch"})
    response2 = client.post("/target_devices", json={"matter_id": "switch.fake-matter-device-2", "name": "Fake Matter Device", "type": "switch"})
    assert b"Device already exists" in response2.data
